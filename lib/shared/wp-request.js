...
function validatePathLevel( levelDefinitions, levelContents ) {
	...
	return levelDefinitions.reduce(function( anyOptionValid, levelOption ) {
...
WPRequest.prototype.validatePath = function() {
	...
	for ( var level = 0; level < maxLevel; level++ ) {
		...
		if ( this._path[ level ] ) {
			...
			if ( ! validatePathLevel( this._levels[ level ], this._path[ level ] ) ) {
				throw new Error([
					'Invalid path component:',
					levelContents,
					'does not match any of',
					this._levels[ level ].reduce(function( components, levelOption ) {
						return components.concat( levelOption ).component;
					}, [] ).join( ', ' ),
					levelOption.component
				].join( ' ' ) );
			}
