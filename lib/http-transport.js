...
/**
 * Attach data to the request, using form encoding to attach an image to the
 * request if an image is provided for upload
 *
 * @param {Object} request A superagent request object
 * @param {Object} data The data for a POST or PUT request
 * @param {WPRequest} wpreq A WPRequest query object
 * @returns {Object} A superagent request object
 */
function attachDataToRequest( request, data, wpreq ) {
	if ( ! wpreq._attachment ) {
		return request.send( data );
	}

	// Data must be form-encoded alongside image attachment
	return _reduce( data, function( req, value, key ) {
...
function _httpPost( wpreq, data, callback ) {
	...
	request = attachDataToRequest( request, data, wpreq );
...
function _httpPut( wpreq, data, callback ) {
	...
	var request = _auth( agent.put( url ), wpreq._options, true );
