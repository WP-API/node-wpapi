...
function getHeaders( url, data, oauth, token ) {
	token = token || null;
...
function _auth( request, options, forceAuthentication ) {
	...
	// if ( options.token && options.secret && options.oauth ) {
	// 	return request
	// 		.set( getHeaders( request.toString(), ));
	// }
	...
	// No auth scheme provided, don't authenticate
	return request;
}
