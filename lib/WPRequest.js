...
WPRequest.prototype._renderURI = function() {
	// If the current path has a custom path template, render the path
	var path = this._path && this._path.template ? this._renderPath() : '';

	// If the current request supports filters, render them to a query string
	var queryStr = this._queryStr ? this._queryStr() : '';
