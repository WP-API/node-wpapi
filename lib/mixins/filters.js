...
/**
 * Determine if a value is a number or a string representing a natural number
 *
 * @private
 * @param {*} val A value to test for numbertude
 * @returns {Boolean} Whether the value can be considered a number
 */
function isNum( val ) {
...
filterMixins.taxonomy = function( taxonomy, term ) {
	...
	var termIsNumber = termIsArray ?
		term.reduce(function( allAreNumbers, term ) {
			return allAreNumbers && isNum( term );
		}, true ) :
		isNum( term );
	var termIsString;
	if ( termIsNumber ) {
