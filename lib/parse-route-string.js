...
Object.keys( routesByNamespace ).forEach(function( namespace, idx ) {
	...
	var endpointHandlers = Object.keys( nsRoutes ).reduce(function( handlers, resource ) {
		...
		function extractSetterFromNode( node ) {
			...
			handler.addLevel( node.level, _.pick( node, 'validate', 'methods' ) );
			handler.addLevelValidator( node.level, node.validate );

			var setterFn = node.namedGroup ? function( val ) {
				handler.setPathPart( node.level, val );
			} : function( val ) {
				handler.setPathPart( node.level, 'GET THIS VALUE SOMEHOW' );
				var dynamicChild = dynamicChildren.length === 1 && dynamicChildren[ 0 ];
				if ( dynamicChild ) {
					handler.setPathPart( dynamicChild.level, val );
				}
			}
