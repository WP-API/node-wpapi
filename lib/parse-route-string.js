...
Object.keys( routesByNamespace ).forEach(function( namespace, idx ) {
	...
	var endpointHandlers = Object.keys( nsRoutes ).reduce(function( handlers, resource ) {
		...
		function extractSetterFromNode( node ) {
			...
			if ( node.level > 0 ) {
				...
				node.names.forEach(function( name ) {
					...
					if ( ! handler._setters[ name ] ) {
						handler._setters[ name ] = setterFn;
					}
				});
			}
		...
		// Walk the tree
		getValues( nsRoutes[ resource ] ).map( extractSetterFromNode );
	...
	if ( endpointHandlers.posts ) {
		console.log(
			...
			}).id( 4 ).revisions( 52 ).validatePath()._renderPath()
		);
		try {
			endpointHandlers.posts().revisions( 52 ).validatePath()._renderPath();
		} catch( e ) { console.error( e ); }
		console.log(
			endpointHandlers.posts().id( 4 ).meta().filter( 'some', 'prop' )._renderPath()
		);
	}
