...
var routesByNamespace = Object.keys( routes ).reduce(function( nsGroups, route ) {
	...
	if ( nsForRoute && '/' + nsForRoute !== route && ! nsGroups[ nsForRoute ] ) {
		nsGroups[ nsForRoute ] = [];
	}
	// Don't add the namespace root to its own group
	if ( '/' + nsForRoute !== route ) {
		// Add this route string to its namespace group, stripping the namespace from
		// the route string
		nsGroups[ nsForRoute ].push( route.replace( '/' + nsForRoute + '/', '' ) );
	}
	return nsGroups;
}, {} );

console.log( routesByNamespace );
/*
