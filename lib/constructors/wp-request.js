 ...
 * @param {String} [options.jwt] A JSON Web Token for use with jwt authentication
 */
 ...
 *     request.auth().get...
 *
 * Alternatively, a username & password (or nonce or jwt) can be explicitly passed into `.auth`:
 *
 ...
 *     })...
 *
 * @example use a token for JSON Web Token authentication
 *
 *     request.auth({
 *       jwt: 'sometoken'
 *     })...
 ...
 *                                        properties, or else a 'nonce' or 'jwt' property
...
WPRequest.prototype.auth = function( credentials ) {
	if ( typeof credentials === 'object' ) {
		...
		if ( credentials.jwt ) {
			this._options.jwt = credentials.jwt;
		}
	}
