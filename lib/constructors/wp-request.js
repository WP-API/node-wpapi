 ...
 * @method
 * @chainable
 * @param {string|Buffer|Blob|File} file A path to a file (in Node), or the file data as a
 *                                       Buffer, Blob or File object, to attach to the request
 * @param {string}                 [name] A filename (with extension) to use for the file;
 *                                        required unless the file is a path or File object
 * @returns {WPRequest} The WPRequest instance (for chaining)
 */
WPRequest.prototype.file = function( file, name ) {
	if ( ! name ) {
		const isBuffer = global.Buffer && file instanceof global.Buffer;
		const isBareBlob = typeof Blob !== 'undefined' && file instanceof Blob &&
